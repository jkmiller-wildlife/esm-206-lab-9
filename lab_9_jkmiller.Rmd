...
# NOTE: This is bad. PricePerSqFt makes no sense to use because it is caluculated from the Price. Get into a feedback loop. Calculated from dependent variable Price. Can lead to biased overfiting of model results. Colinearity.

# When coefficients make no sense, perhaps one of the variables that is highly correlated (like PricePerSqFt) is throwing it off. Pick one variable of multiple variables that are essentially telling you the same thing. 

# Either pick the best one or aggregate them into a new combined variable.

# Bedrooms, Bathrooms, and SqFt are essentially telling the same thing. House size.
...
# I would still pick homes_lm3 because: # Overall, heteroscedasticity is OK, residuals normality definitely looks good, and conceptually and mathematically my model is making sense!
...
predict_df <- data.frame(df_new, price_predict) # Combines columns as long as same number of rows.
